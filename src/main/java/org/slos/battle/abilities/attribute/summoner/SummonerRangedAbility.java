...
public class SummonerRangedAbility extends Ability implements BuffAbility {
    int buffQuantity;

    public SummonerRangedAbility(int buffQuantity) {
        super(AbilityType.SUMMONER_RANGED, AbilityClassification.BUFF);
        this.buffQuantity = buffQuantity;
    }

    @Override
    public Buff getBuffEffect() {
        return new BattleAttributeBuff(BattleAttributeType.RANGED, buffQuantity);
    }
